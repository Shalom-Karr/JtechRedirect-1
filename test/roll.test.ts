...
function start(options: { seed: number; reduceMotion?: boolean }): Harness {
  ...
  const code = readFileSync(BUNDLE, "utf8").replace(
    "__SITE_DATA__",
    JSON.stringify({ themes: THEMES, initialThemeIndex: 19, taglines: TAGLINES }),
  );
...
test("opens on a tagline, in both the visible text and the live region", () => {
  const page = start({ seed: 1_700_000_000_000 });

  assert.ok(TAGLINES.includes(page.tagline.textContent));
  assert.equal(page.announcer.textContent, page.tagline.textContent);
});

test("scrambles on the way over, then lands exactly on the next line", () => {
  const page = start({ seed: 1_700_000_000_000 });
  const before = page.tagline.textContent;

  const painted = page.advance(SETTLED_MS + 7000);
...
test("leaves the word gaps alone so the line still reads as words", () => {
  const page = start({ seed: 1_700_000_000_042 });
  const painted = page.advance(SETTLED_MS + 7000);
...
test("holds the announcement back until the line has settled", () => {
  const page = start({ seed: 1_700_000_000_007 });
  const opening = page.announcer.textContent;

  // Into the first changeover, but not through it.
  page.advance(7000 + 200);
  assert.equal(page.announcer.textContent, opening, "announced before it settled");

  page.advance(SETTLED_MS);
  assert.equal(page.announcer.textContent, page.tagline.textContent);
  assert.ok(TAGLINES.includes(page.announcer.textContent));
});

test("swaps outright when the visitor asked for reduced motion", () => {
  const page = start({ seed: 1_700_000_000_000, reduceMotion: true });
  const before = page.tagline.textContent;
...
test("a different clock gives a different walk, and the walk covers everything", () => {
  const walk = (seed: number): string[] => {
    const page = start({ seed });
  ...
  const first = walk(1_700_000_000_000);
  const second = walk(1_700_000_555_123);
...
test("cycles the theme when the banner is clicked", () => {
  const page = start({ seed: 1_700_000_000_000 });
